...
class YuyulearningPlugin extends UserPluginBase
{
    ...
    public function adminProgress($request,$page_id,$frame_id,$course_id)
    {
        ...
        return $this->view('yuyu_learning_admin_progress',[
            'page_id'=>$page_id,
            'frame_id'=>$frame_id,
            'course'=>$course,
            'report'=>$report,
    ...
    public function listCourses($request,$page_id,$frame_id){$yuyu_learning_frame=$this->getOrCreateYuyuLearningFrame($frame_id);$courses=YuyuLearningCourse::query()->orderBy('sort_order')->orderBy('id')->get();return $this->view('yuyu_learning_course_list',compact('page_id','frame_id','courses')+['selected_course_id'=>$yuyu_learning_frame->course_id])->withInput($request->all());}
    public function selectCourse($request,$page_id,$frame_id){$validator=Validator::make($request->all(),['course_id'=>['required','integer','exists:yuyu_learning_courses,id']]);$validator->setAttributeNames(['course_id'=>'コース']);if($validator->fails())return redirect()->back()->withErrors($validator)->withInput();$yuyu_learning_frame=$this->getOrCreateYuyuLearningFrame($frame_id);$yuyu_learning_frame->course_id=(int)$request->input('course_id');$yuyu_learning_frame->save();$request->flash_message='このフレームで使用するコースを変更しました。';$request->merge(['redirect_path'=>$request->input('normal_page_path')]);}
    public function createCourse($request,$page_id,$frame_id){return $this->view('yuyu_learning_course_create',['page_id'=>$page_id,'frame_id'=>$frame_id,'sort_order'=>(int)YuyuLearningCourse::max('sort_order')+1])->withInput($request->all());}
    public function editCourse($request,$page_id,$frame_id,$course_id){$course=YuyuLearningCourse::findOrFail($course_id);$course->load(['sections'=>function($query){$query->with(['contents'=>function($contents){$contents->orderBy('sort_order')->orderBy('id');}])->orderBy('sort_order')->orderBy('id');}]);return $this->view('yuyu_learning_course_edit',['course'=>$course,'is_create'=>false])->withInput($request->all());}
    ...
    public function createSection($request,$page_id,$frame_id,$course_id){$course=YuyuLearningCourse::findOrFail($course_id);$section=new YuyuLearningSection(['course_id'=>$course->id,'sort_order'=>(int)YuyuLearningSection::where('course_id',$course->id)->max('sort_order')+1]);return $this->view('yuyu_learning_section_edit',['course'=>$course,'section'=>$section,'is_create'=>true])->withInput($request->all());}
    public function editSection($request,$page_id,$frame_id,$section_id){$section=YuyuLearningSection::with('course')->findOrFail($section_id);return $this->view('yuyu_learning_section_edit',['course'=>$section->course,'section'=>$section,'is_create'=>false])->withInput($request->all());}
