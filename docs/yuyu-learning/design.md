...
| --- | --- | --- |
| コース管理 | コンテンツ管理者（`role_article_admin`） | 受講進捗とForms登録一覧を閲覧し、コース・章・教材を作成・編集して、自ら公開できる。 |
| 受講進捗閲覧 | モデレータ（`role_article`） | 受講進捗を閲覧できる。コース編集とForms登録一覧は利用できない。 |
| 承認付きコース作成 | 編集者（`role_reporter`） | 自分のコースを作成・編集できる。公開には承認が必要。 |
...
### 4.8 `yuyu_learning_annotations`

固定記事教材・ブログ教材に付ける、受講者本人専用の注釈を管理する。元の記事本文および既存プラグインのテーブルは更新しない。

- `id`
- `user_id`
- `content_id`
- `blog_post_id` nullable
- `annotation_type`: `highlight / note`
- `color` nullable: `yellow / green / blue / pink`
- `selected_text`
- `prefix_text` nullable
- `suffix_text` nullable
- `start_offset`
- `end_offset`
- `note` nullable
- timestamps

`user_id` と `content_id` に複合インデックスを設定する。ユーザーまたは教材を削除した場合は、対応する注釈もcascade deleteする。

固定記事教材では教材フレーム先頭、ブログ教材では `blog_post_id` で特定した各記事の先頭からの文字オフセットを保存する。ブログの表示件数、並び順、ページ送りが変わっても、記事内の位置は影響を受けない。

記事編集後の再探索に備えて、選択文字列と前後各80文字も保存する。オフセットで一致しない場合は前後文脈、選択文字列の順に再探索する。特定できない注釈は削除せず、注釈一覧に「位置を確認できません」と表示する。
...
注釈機能はWYSIWYGエディタや固定記事・Blogs本体へ追加しない。YuyuLearningのコース目次から同一オリジンの固定記事教材・ブログ教材を開いた場合だけ、親画面のJavaScriptが教材ウィンドウへ注釈ツールを挿入する。通常のページ閲覧および記事編集では動作しない。
...
- 吹き出しアイコンからのメモ編集
- 注釈の削除
- マーカーと吹き出しを一時的に隠す「注釈を隠す／注釈を表示」切替
- マーカークリック時の色変更・削除
- 吹き出しクリック時のメモ編集・削除

「マーカー」または「メモ」を選んだ後だけ、教材本文の文字列選択を注釈登録として扱う。通常時の文字列選択やコピーは妨げない。対象DOMは登録された教材フレームの `#frame-{frame_id}` 内に限定する。

ブログ一覧では、各 `<article>` 内の個別記事リンクからブログ記事IDを判定する。ブログ個別表示ではURLから記事IDを判定する。現在の画面に表示されている記事IDだけを注釈APIへ渡し、表示中の記事の注釈だけを取得・復元する。1つの選択範囲を複数記事へまたがせることはできない。

「注釈を隠す」を選択した場合は、本文を維持したままマーカーの背景色と吹き出しアイコンだけを非表示にする。注釈データは削除しない。非表示中は注釈一覧を閉じ、マーカー・メモの登録モードも解除する。

マーカー色は教材本文上で判別しやすい彩度とし、マーカーにはクリック可能であることが分かる表示を付ける。吹き出しは濃い暖色、太い枠、大きめのアイコンで表示する。

注釈APIはログイン、教材種別、対象コースの受講実績、`user_id` の一致を毎回検証する。他の受講者の注釈は取得・変更できない。第1版では注釈の共有、管理者による一括閲覧、重なり合う注釈、丸囲みおよび自由描画を対象外とする。

注釈の一覧・保存・削除は、CMS画面全体を返す `/plugin` ではなく、プラグインの戻り値を直接返すConnect-CMS標準の `/json/yuyulearning/...` ルートを使用する。
